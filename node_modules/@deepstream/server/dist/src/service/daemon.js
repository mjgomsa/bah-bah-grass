...
function _start(options) {
        ...
        if (!child.pid) {
            ...
            if (starts >= options.maxRetries) {
                if ((Date.now() - startTime) > maxMilliseconds) {
                    console.error(`Too many restarts within the last ${maxMilliseconds / 1000} seconds. Please check the script.`);
                    process.exit(1);
                }
            }
            setTimeout(() => {
                ...
                if (attempts > options.maxRestarts && options.maxRestarts >= 0) {
                    console.error(`${options.name} will not be restarted because the maximum number of total restarts has been exceeded.`);
                    process.exit();
                }
                else {
                    launch();
    ...
    function launch() {
        ...
        child = child_process_1.spawn(options.processExec, ['start'].concat(process.argv.slice(2)), {
            env: process.env
        });
