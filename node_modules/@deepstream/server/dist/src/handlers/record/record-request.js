...
exports.recordRequestBinding = exports.recordRequest = void 0;
...
function sendError(event, errorMessage, recordName, socketWrapper, onError, services, context, metaData, message) {
    services.logger.error(constants_1.RECORD_ACTION[event], errorMessage, metaData);
    if (message) {
 ...
 */
function onStorageResponse(error, recordName, version, data, socketWrapper, onComplete, onError, services, context, metaData, promoteToCache, message) {
    if (error) {
        sendError(constants_1.RECORD_ACTION.RECORD_LOAD_ERROR, `error while loading ${recordName} from storage:${error}`, recordName, socketWrapper, onError, services, context, metaData, message);
    }
    else {
        if (message) {
 ...
 */
function onCacheResponse(error, recordName, version, data, socketWrapper, onComplete, onError, config, services, context, metaData, promoteToCache, message) {
    if (error) {
        sendError(constants_1.RECORD_ACTION.RECORD_LOAD_ERROR, `error while loading ${recordName} from cache:${error}`, recordName, socketWrapper, onError, services, context, metaData, message);
    }
    else if (data) {
        if (message) {
    ...
    else if (!utils_1.isExcluded(config.record.storageExclusionPrefixes, recordName)) {
        let storageTimedOut = false;
        const storageTimeout = setTimeout(() => {
            storageTimedOut = true;
            sendError(constants_1.RECORD_ACTION.STORAGE_RETRIEVAL_TIMEOUT, recordName, recordName, socketWrapper, onError, services, context, metaData, message);
        }, config.record.storageRetrievalTimeout);
        // tslint:disable-next-line:no-shadowed-variable
        services.storage.get(recordName, (storageError, version, result) => {
            if (!storageTimedOut) {
                clearTimeout(storageTimeout);
                onStorageResponse(storageError, recordName, version, result, socketWrapper, onComplete, onError, services, context, metaData, promoteToCache, message);
            }
        }, metaData);
    }
    else {
 ...
 */
function recordRequest(recordName, config, services, socketWrapper, onComplete, onError, context, metaData, message, promoteToCache = true) {
    let cacheTimedOut = false;
    const cacheTimeout = setTimeout(() => {
        cacheTimedOut = true;
        sendError(constants_1.RECORD_ACTION.CACHE_RETRIEVAL_TIMEOUT, recordName, recordName, socketWrapper, onError, services, context, metaData, message);
    }, config.record.cacheRetrievalTimeout);
    services.cache.get(recordName, (error, version, data) => {
        if (!cacheTimedOut) {
            clearTimeout(cacheTimeout);
            onCacheResponse(error, recordName, version, data, socketWrapper, onComplete, onError, config, services, context, metaData, promoteToCache, message);
        }
    }, metaData);
}
