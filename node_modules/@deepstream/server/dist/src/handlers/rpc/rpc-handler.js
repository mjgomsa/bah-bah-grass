...
class RpcHandler extends types_1.Handler {
    /**
    ...
    */
    constructor(config, services, subscriptionRegistry, metaData) {
        ...
        this.subscriptionRegistry =
            subscriptionRegistry || services.subscriptions.getSubscriptionRegistry(constants_1.TOPIC.RPC, constants_1.STATE_REGISTRY_TOPIC.RPC_SUBSCRIPTIONS);
        this.subscriptionRegistry.setAction('NOT_SUBSCRIBED', constants_1.RPC_ACTION.NOT_PROVIDED);
        this.subscriptionRegistry.setAction('MULTIPLE_SUBSCRIPTIONS', constants_1.RPC_ACTION.MULTIPLE_PROVIDERS);
        this.subscriptionRegistry.setAction('SUBSCRIBE', constants_1.RPC_ACTION.PROVIDE);
        this.subscriptionRegistry.setAction('UNSUBSCRIBE', constants_1.RPC_ACTION.UNPROVIDE);
    }
    /**
    * Main interface. Handles incoming messages
    * from the message distributor
    */
    handle(socketWrapper, message, originServerName) {
        ...
        if (message.action === constants_1.RPC_ACTION.PROVIDE) {
            this.subscriptionRegistry.subscribeBulk(message, socketWrapper);
            return;
        }
        if (message.action === constants_1.RPC_ACTION.UNPROVIDE) {
            this.subscriptionRegistry.unsubscribeBulk(message, socketWrapper);
            return;
        }
        if (message.action === constants_1.RPC_ACTION.RESPONSE ||
            message.action === constants_1.RPC_ACTION.REJECT ||
            message.action === constants_1.RPC_ACTION.ACCEPT ||
            message.action === constants_1.RPC_ACTION.REQUEST_ERROR) {
            const rpcData = this.rpcs.get(message.correlationId);
            if (rpcData) {
                this.services.logger.debug(constants_1.RPC_ACTION[message.action], `name: ${message.name} with correlation id: ${message.correlationId} from ${socketWrapper.userId}`, this.metaData);
                rpcData.rpc.handle(message);
                return;
            }
            this.services.logger.warn(constants_1.RPC_ACTION[constants_1.RPC_ACTION.INVALID_RPC_CORRELATION_ID], `name: ${message.name} with correlation id: ${message.correlationId}`, this.metaData);
            socketWrapper.sendMessage({
                topic: constants_1.TOPIC.RPC,
                action: constants_1.RPC_ACTION.INVALID_RPC_CORRELATION_ID,
        ...
        */
        this.services.logger.warn(constants_1.PARSER_ACTION[constants_1.PARSER_ACTION.UNKNOWN_ACTION], message.action.toString(), this.metaData);
    }
    /**
    ...
    getAlternativeProvider(rpcName, correlationId) {
        ...
        for (let n = 0; n < servers.length; ++n) {
            if (!rpcData.servers.has(servers[index])) {
                ...
                return new rpc_proxy_1.RpcProxy(this.config, this.services, servers[index], this.metaData);
            }
    ...
    */
    makeRpc(socketWrapper, message, isRemote) {
        ...
        this.services.logger.debug(constants_1.RPC_ACTION[constants_1.RPC_ACTION.REQUEST], `name: ${rpcName} with correlation id: ${correlationId} from ${socketWrapper.userId}`, this.metaData);
        const subscribers = Array.from(this.subscriptionRegistry.getLocalSubscribers(rpcName));
        ...
        if (provider) {
            this.rpcs.set(correlationId, {
                ...
                rpc: new rpc_1.Rpc(this, socketWrapper, provider, this.config, message),
            });
            return;
        }
        if (isRemote) {
    ...
    */
    makeRemoteRpc(requestor, message) {
        ...
        if (server) {
            const rpcProxy = new rpc_proxy_1.RpcProxy(this.config, this.services, server, this.metaData);
            this.rpcs.set(correlationId, {
                providers: new Set(),
                servers: new Set(),
                rpc: new rpc_1.Rpc(this, requestor, rpcProxy, this.config, message),
            });
            return;
        }
        this.rpcs.delete(correlationId);
        this.services.logger.warn(constants_1.RPC_ACTION[constants_1.RPC_ACTION.NO_RPC_PROVIDER], `name: ${message.name} with correlation id: ${message.correlationId}`, this.metaData);
    ...
    */
    onRemoteRPCMessage(msg, originServerName) {
        if (msg.action === constants_1.RPC_ACTION.REQUEST) {
            const proxy = new rpc_proxy_1.RpcProxy(this.config, this.services, originServerName, this.metaData);
            this.makeRpc(proxy, msg, true);
            return;
        }
        const rpcData = this.rpcs.get(msg.correlationId);
        if (!rpcData) {
            this.services.logger.warn(constants_1.RPC_ACTION[constants_1.RPC_ACTION.INVALID_RPC_CORRELATION_ID], `Message bus response for RPC that may have been destroyed: ${JSON.stringify(msg)}`, this.metaData);
            return;
        }
        this.services.logger.debug(constants_1.RPC_ACTION[msg.action], `name: ${msg.name} with correlation id: ${msg.correlationId} from remote server ${originServerName}`, this.metaData);
