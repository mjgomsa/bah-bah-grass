...
const fileUtil = require("../src/config/file-utils");
const nginx = (program) => {
    program
        .command('nginx')
...
`;
    const websocketConfig = dsConfig.connectionEndpoints.reduce((result, endpoint) => {
    ...
    let httpConfig = '';
    const http = dsConfig.connectionEndpoints.find((endpoint) => endpoint.type === 'http');
    if (http) {
...
http {
    ...
    if (this.output) {
        fs_1.writeFileSync(fileUtil.lookupConfRequirePath(this.output), config, 'utf8');
...
//# sourceMappingURL=deepstream-nginx.js.map